...
叩くたびに **WEAK / GOOD / PERFECT / DANGER** が出ます。

| 判定 | 内容 |
| --- | --- |
| WEAK | 弱すぎ。少しズレるだけで抜けきらない。続けると塔が傾いて危ない |
| GOOD | ちゃんと抜ける。**クリアだけなら GOOD で十分です** |
| PERFECT | 緑帯の中心にある金色の芯。抜けかたは GOOD と同じですが、**コンボが続くのはここだけ** |
| DANGER | 強すぎ。ブロックが暴れて飛び出し、上の段と雷神も大きく揺れる |

ゲージの緑が GOOD、その中心の金色が PERFECT の芯です。

### 難易度カーブ

**段を抜くほど、カーソルが速くなり、判定の帯が狭くなります。**
ステージ（神社・屋上…）では変わりません。1プレイの中で少しずつ効いてきます。

| 抜いた段 | ゲージ往復 | GOOD幅 | PERFECT幅 | PERFECTに居られる時間 |
| --- | --- | --- | --- | --- |
| 0（序盤） | 1.60秒 | 42% | 19.3% | 155ms |
| 5（中盤） | 1.29秒 | 37.3% | 14.7% | 95ms |
| 9（終盤） | 1.04秒 | 33.6% | 11.4% | 59ms |

終盤はカーソルの速さにわずかなゆらぎも入るので、リズムだけでは芯を取りにくくなります
（完全ランダムではなく、見て反応できる程度のゆっくりした変化です）。
金のコマはボーナスなので、この難易度カーブの対象外です。

### 妨害モード

段が進むと、溜めている最中にじゃまが入るようになります。
**どれも必ず 0.7 秒の予告が先に出ます。**「見て、覚えて、予測すれば取れる」範囲に収めてあり、
予兆なしにカーソルの速さが変わることはありません。同時に起きるのは**最大2種類**までです。

| 表示 | 内容 | 解禁 |
| --- | --- | --- |
| `DARK CLOUD` | バーの一部が雲で隠れる。カーソルは同じ速さで動き続けるので、入る前の位置と速さから読む | 3段 |
| `ONE CHANCE` | 1往復のうちに離さないと弱い打撃になる | 4段 |
| `MOVING ZONE` | PERFECT の芯だけがゆっくり左右する。GOOD の範囲は動かない | 6段 |
| `BLACKOUT` | バー全体が 0.3〜0.55 秒だけ見えなくなる | 6段 |
| `FEINT` | 速さが 0.62倍／1.45倍に変わる。**変わる 0.35 秒前に必ずバーが光って音が鳴る** | 8段 |

発生率は 3段で25%、6段で50%、9段で82%。2種同時は7段から出はじめ、終盤で28%ほどです。
「見えない」もの同士（暗雲＋停電、一発勝負＋停電）は重なりません。
金のコマには妨害は出ません。

PERFECT が続くと **コンボ**が伸びます。PERFECT 以外はその場でリセットです
（`COMBO GUARD` があれば1回だけ守られます）。
...
| 難易度カーブ（速さ・帯の幅） | `hit.difficulty.*` の `start` / `end` |
| 妨害モードの解禁・発生率 | `hit.hazards.unlock` / `hit.hazards.chance` |
| 妨害モードを止める | `hit.hazards.enabled` を `false` に |
